...
public class CreativeTabAPI {
	...
	private static final HashMap<ResourceKey<CreativeModeTab>, FrozenEvent<ModifyEntries>> EVENT_MAP = new HashMap<>();

	public static @NotNull FrozenEvent<ModifyEntries> modifyEntriesEvent(ResourceKey<CreativeModeTab> registryKey) {
		if (!EVENT_MAP.containsKey(registryKey)) EVENT_MAP.put(registryKey, FrozenEvent.createEvent(ModifyEntries.class));
		return EVENT_MAP.get(registryKey);
	}
	...
	public static void addBefore(ItemLike comparedItem, ItemLike item, CreativeModeTab.TabVisibility tabVisibility, ResourceKey<CreativeModeTab> @NotNull ... tabs) {
		...
		for (ResourceKey<CreativeModeTab> tab : tabs) {
			...
			modifyEntriesEvent(tab).register(entries -> entries.addBefore(comparedItem, list, tabVisibility));
		}
	}
	...
	public static void addAfter(ItemLike comparedItem, ItemLike item, CreativeModeTab.TabVisibility tabVisibility, ResourceKey<CreativeModeTab> @NotNull ... tabs) {
		...
		for (ResourceKey<CreativeModeTab> tab : tabs) {
			...
			stack.setCount(1);
			List<ItemStack> list = List.of(stack);
			modifyEntriesEvent(tab).register((entries) -> entries.addAfter(comparedItem, list, tabVisibility));
		}
	}
