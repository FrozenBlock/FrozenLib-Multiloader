...
public class TooltipListEntryMixin {
	...
	public Optional<Component[]> frozenLib$getTooltip(Optional<Component[]> list) {
		...
		if (!disableableWidgetInterface.frozenLib$getEntryPermissionType().canModify) {
			Optional<Component> optionalComponent = FrozenLibClientNetworking.connectedToLan() ?
			...
			return list.map(components -> Stream.concat(Arrays.stream(components), Stream.of(entrySyncNotice)).toArray(Component[]::new)).or(() -> Optional.of(entrySyncNotice.toFlatList().toArray(new Component[0])));
		}
		return list;
	}
