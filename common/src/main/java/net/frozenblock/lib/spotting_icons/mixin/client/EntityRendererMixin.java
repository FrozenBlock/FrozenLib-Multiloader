...
public abstract class EntityRendererMixin<T extends Entity> implements EntityRendererWithIcon {
	...
	public <T extends Entity> void frozenLib$renderIcon(T entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
		if (entity instanceof EntitySpottingIconInterface spottingIconInterface) {
			SpottingIconManager iconManager = spottingIconInterface.frozenLib$getSpottingIconManager();
			...
			if (icon != null) {
				...
				if (dist > icon.startFadeDist() && iconManager.clientHasIconResource) {
					...
					poseStack.pushPose();
					poseStack.translate(0F, f, 0F);
					poseStack.mulPose(this.entityRenderDispatcher.cameraOrientation());
					poseStack.scale(-1F, 1F, 1F);
					PoseStack.Pose pose = poseStack.last();
					VertexConsumer vertexConsumer = buffer.getBuffer(
						FrozenLibRenderTypes.entityTranslucentEmissiveAlwaysRender(icon.texture())
					);
					frozenLib$vertex(vertexConsumer, pose, packedLight, 0F, 0, 0, 1, alpha);
					frozenLib$vertex(vertexConsumer, pose, packedLight, 1F, 0, 1, 1, alpha);
					frozenLib$vertex(vertexConsumer, pose, packedLight, 1F, 1, 1, 0, alpha);
					frozenLib$vertex(vertexConsumer, pose, packedLight, 0F, 1, 0, 0, alpha);
					poseStack.popPose();
				}
			}
		}
	...
	private static void frozenLib$vertex(@NotNull VertexConsumer vertexConsumer, PoseStack.Pose pose, int i, float f, int j, int u, int v, int alpha) {
		vertexConsumer.addVertex(pose, f - 0.5F, (float)j - 0.5F, 0F)
