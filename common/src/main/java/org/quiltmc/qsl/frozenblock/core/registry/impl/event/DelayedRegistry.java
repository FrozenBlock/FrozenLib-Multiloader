...
public abstract class DelayedRegistry<T> implements WritableRegistry<T> {
	protected final WritableRegistry<T> wrapped;
	private final Queue<DelayedEntry<T>> delayedEntries = new LinkedList<>();

	protected DelayedRegistry(WritableRegistry<T> registry) {
	...
	public Reference<T> register(ResourceKey<T> key, T entry, RegistrationInfo registrationInfo) {
		...
		return Reference.createStandAlone(this.wrapped.holderOwner(), key);
	}
